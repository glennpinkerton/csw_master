...
import csw.jsurfaceworks.src.DBPoint;
import csw.jsurfaceworks.src.DBPointList;
import csw.jsurfaceworks.src.DBContactLine;
...
class SurfSplitFrame extends JFrame
{
    ...
    private ArrayList<CLine>       clineList = new ArrayList<CLine> (20);
...
/*----------------------------------------------------------------------------*/
    ...
    private void calcPolygonsFromCLines ()
    {
        ...
        ArrayList<DBPoint> alist = jsw.getPolyLabelList ();
        drawPolyLabels (alist);

        drawWorkPolySurfaces ();

        ArrayList<DBContactLine> ctlist = jsw.getProtoPatchContactLineList ();
        drawContactLines (ctlist);

        repaint ();

    }

    private void drawPolyLabels (ArrayList<DBPoint> list)
    {
        int           i, n;
        DBPoint       p;
        ...
        for (i=0; i<n; i++) {
            p = list.get (i);
            dl.addNumber (p.x, p.y, .15, 0.0, p.ival1, 0, 0);
        }
    ...
    private void drawResultList (ArrayList<XYZPolyline> list)
    {
        XYZPolyline      line;
        double[]       xp, yp;
        int            lsize, i, j, np;
    ...
    private void drawContactLines (ArrayList<DBContactLine> alist)
    {
        ...
        for (int i=0; i<size; i++) {
            ...
            yp = dbl.getYArray ();
            np = dbl.getNpts();
            dpid1 = (double) dbl.getPatchID1();
            dl.addLine (xp, yp, np);
            dl.addNumber (xp[0], yp[0], 0.1, 0.0, dpid1, 0, 0);
        }

    }


    private void drawWorkPolyList (ArrayList<XYZPolyline> list)
    {
        XYZPolyline      line;
        double[]       xp, yp;
        int            lsize, i, np;
    ...
    private void drawFillLabels (ArrayList<XYZPolyline> list)
    {
        XYZPolyline      line;
        double[]       xp, yp;
        int            lsize, i, j, np;
