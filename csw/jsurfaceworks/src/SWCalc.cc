    ...
    SWGridOptionsStruct *grid_options)
{
    ...
    CSW_Mixture mix;
    CSW_Float range;
    CSW_BlendedNode blend;
    FAultLineStruct *boundaries;
    int nBoundaries;
    int do_write;

    /* allocate new arrays */
    nNodes = ncols * nrows;
    blendedNodes = (CSW_BlendedNode *) calloc(
        1,
        nNodes *
        sizeof(CSW_BlendedNode)
    );
    if (blendedNodes == NULL) {
        return -1;
    }

    do_write = csw_GetDoWrite ();

    /* Load boundary data into a FAultLineStruct */
    if (nfaults > 0) {
      istat = grdapi_ptr->grd_DoubleFaultArraysToStructs(
        xfault,
        yfault,
        NULL, /* z faults */
        nFaultPts,
        NULL, /* fault line types */
        nfaults,
        &boundaries,
        &nBoundaries
      );
      if (do_write) {
        grdapi_ptr->grd_WriteFaultLines (boundaries, nBoundaries,
                             (char *)"fault1.xyz");
      }
     ...
     * Transfer grid options.
     */
    ConvertGridCalcOptions (grid_options, &options);
    istat = grdapi_ptr->grd_CalcBlendedGridFromDouble(
        x,
        y,
        z,
        npts,
    ...
    if (do_write) {
      grdapi_ptr->grd_WriteBlendedGrid (
        blendedNodes, ncols, nrows, 1,
        xmin, ymin, xmax, ymax,
        (char *)"blend3.xyz");
    }

    grdapi_ptr->grd_FreeFaultLineStructs (boundaries, nBoundaries);
    boundaries = NULL;
    nBoundaries = 0;

    values1 = (int *)malloc (nNodes * sizeof(int));
    if (values1 == NULL) {
        free(blendedNodes);
        return -1;
    }
    values2 = (int *)malloc (nNodes * sizeof(int));
    if (values2 == NULL) {
        free(blendedNodes);
        free(values1);
        return -1;
    }
    values3 = (int *)malloc (nNodes * sizeof(int));
    if (values3 == NULL) {
        free(blendedNodes);
        free(values1);
        free(values2);
    ...
    values4 = (int *)malloc (nNodes * sizeof(int));
    if (values4 == NULL) {
        free(blendedNodes);
        free(values1);
        free(values2);
    ...
    fractions1 = (double *)malloc (nNodes * sizeof(double));
    if (fractions1 == NULL) {
        free(blendedNodes);
        free(values1);
        free(values2);
    ...
    fractions2 = (double *)malloc (nNodes * sizeof(double));
    if (fractions2 == NULL) {
        free(blendedNodes);
        free(values1);
        free(values2);
    ...
    fractions3 = (double *)malloc (nNodes * sizeof(double));
    if (fractions3 == NULL) {
        free(blendedNodes);
        free(values1);
        free(values2);
    ...
    fractions4 = (double *)malloc (nNodes * sizeof(double));
    if (fractions4 == NULL) {
        free(blendedNodes);
        free(values1);
        free(values2);
    ...
    range = 1.0f;
    for (i = 0;i < nNodes; i++) {
        blend = blendedNodes[i];
        grdapi_ptr->grd_BlendedNodeToMixture (&blend, range, &mix);
        values1[i] = mix.value1;
        values2[i] = mix.value2;
        values3[i] = mix.value3;
        values4[i] = mix.value4;
        fractions1[i] = mix.percent1;
        fractions2[i] = mix.percent2;
        fractions3[i] = mix.percent3;
        fractions4[i] = mix.percent4;
    }

    jni_call_add_blended_nodes_method
    (
        values1,
        values2,
        values3,
