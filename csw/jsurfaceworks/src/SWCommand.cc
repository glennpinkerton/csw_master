...
/*----------------------------------------------------------------------------------------*/
     ...
     */
        case SW_PS_ADD_CONNECTING_FAULT:
            ...
            break;
    /*-----------------------------------------------------------------------
     * Calculate a blended grid.
     *
     *  ilist[0] is number of data points (numDataPoints)
     *  ilist[1] is number of rows in calculated blended grid
     *  ilist[2] is number of columns in calculated blended grid
     *  ilist[3] is number of boundaries (numBoundaries)
     *  ilist[4] is total number of boundary points (totalNumBoundaryPoints)
     *  ilist[5] starts number of points for each boundary
     *  dlist[0] is x minimum for grid
     *  dlist[1] is y minimum for grid
     *  dlist[2] is x maximum for grid
     *  dlist[3] is y maximum for grid
     *  ddata[0] starts xlist for nodes
     *  ddata[numDataPoints] starts ylist
     *  ddata[2*numDataPoints] starts xBoundary
     *  ddata[2*numDataPoints +totalNumBoundaryPoints] starts yBoundary
     *  idata[0] starts zList
     */
        case SW_CALC_BLENDED_GRID:
            ...
            if (LogFile) {
                ...
                for (i=0; i<nline; i++) {
                    sprintf (LogFileLine,
                             "%.15e %.15e\n",
                             ddata[i+start], ddata[i+nline+start]
                            );
                    fputs(LogFileLine, LogFile);
                }
            ...
            npts = ilist[0];
            if (ilist[3] > 0) { /* nBoundaries > 0*/
              istat = swcalc->sw_CalcBlendedGrid (
                  ddata,
                  ddata + npts,
                  idata,
                  ddata + (2 * npts),
                  ddata + (2 * npts) + ilist[4],
                  ilist + 5,
                  npts,
                  ilist[3],
                  ilist[1],
...
*/
    ...
    /*--------------------------------------------------------------
     * Add a point set for an NDP attribute to be draped onto the
     * active attribute trimesh.
     *
     *  ilist[0] has npts
     *  ilist[1] has the id of the point set
     *  ilist[2] has the background ndp value
     *  ddata[0 to npts-1] has the x coordinates
     *  ddata[npts to 2*npts-1] has the y coordinates
     *  ddata[2*npts to 3*npts-1] has the ndp values
     */
