...
import jcore.geometry.NodeArray;
import jcore.geometry.NodeArrayElevation;
import jcore.geometry.XYZPolyline;
import jcore.geometry.XYZPolylineList;
 ...
 * It differs from SplitTest.java only that it moves the contrived surfaces
 * horizontally so a reverse fault can be easily picked.
 *
 *  @author Glenn Pinkerton
 *
*/
public class SplitTestR {
    ...
    public static void main (String[] args) {
        ...
        SplitTestRFrame frame = new SplitTestRFrame ();
...
class SplitTestRFrame extends JFrame
    implements J3DSelectListener
{
    ...
    public SplitTestRFrame ()
    {

        final Simple3DFrameR frame = new Simple3DFrameR ();
...
class Simple3DFrameR extends JFrame
   implements J3DLineEditListener, J3DRightClickListener
{
    ...
    private ArrayList commonLines = null;

    private ArrayList slist = null;
    private ArrayList surfPointsList = null;
    private String    sname = null;

    public Simple3DFrameR ()
    {
        ...
        surfPointsList = new ArrayList(20);
        slist = new ArrayList(20);
        commonLines = new ArrayList(20);
                        ...
                        boolean faultFlag)
    {
        ...
        NodeArrayElevation na = new NodeArrayElevation (xpts, ypts, zpts);
    ...
    private ArrayList       faultList = new ArrayList (1);
...
/*----------------------------------------------------------------------*/
    ...
    {
        ArrayList hlist = panel.getSelectedHorizons ();
        if (hlist == null) {
            return;
        }
     ...
     */
        hp = (J3DHorizonPatch)hlist.get (0);
     ...
     */
        ...
        for (int i=0; i<size; i++) {
            ...
            if (pair.sp1 == hp  &&  pair.sp2 != null) {
                XYZPolylineList plist = j3dline.getDrapedLineList ();
                ...
                SurfPointsR spt = findSurfPoints (hp);
     ...
     */
        XYZPolylineList cl = jsw.getExtendedCenterlineList ();
     //   s2d.drawExtendedCenterlines (cl);

        XYZPolylineList pl = jsw.getWorkPolyList ();
     //   s2d.drawWorkPolys (pl);

        ArrayList clist = jsw.getProtoPatchContactLineList ();
        drawContactLines (clist);

        ArrayList dlist = jsw.getDrapeLineList ();
        //drawDrapeLines (dlist);

        ArrayList eflist = jsw.getEfaultTmeshList ();
     //   drawExtendedFaults (eflist);

     // hp.setOpaque (0.5);
        hp.setVisible (false);

        ArrayList prlist = jsw.getProtoPatchList ();
...
/*--------------------------------------------------------------------------*/
    ...
    public void showEditSurface (boolean draw)
    {
        ...
        XYZPolyline        la;
        XYZPolylineList    lalist, dlist;

        J3DLine   jl;
        int size = selectedLines.size();
        lalist = new XYZPolylineList (size);
        for (i=0; i<size; i++) {
...
/*--------------------------------------------------------------------------*/
    ...
    void createInitialSurfaces ()
    {
        ...
        sname = "top surface";
        createInitialSurface (0.0, 0.0, color, false);

        color = new Color (255, 0, 0, 200);
        sname = "middle surface";
        createInitialSurface (20.0, -20.0, color, false);

        color = new Color (0, 0, 255, 200);
        sname = "bottom surface";
        createInitialSurface (40.0, -50.0, color, false);
    ...
    void createInitialSurface (double xorig, double zmid,
                        Color color, boolean faultFlag)
    {
        ...
        SurfPointsR spt = new SurfPointsR ();
        spt.x = x;
        spt.y = y;
        spt.z = z;
        spt.npts = n;
...
/*-------------------------------------------------------------*/
    ...
    private SurfPointsR findSurfPoints (J3DSurfacePatch sp)
    {
        SurfPointsR spt;

        int size = surfPointsList.size ();
        for (int i=0; i<size; i++) {
            spt = (SurfPointsR)surfPointsList.get (i);
            if (spt.sp.equals(sp)) {
                return spt;
            }
        }
...
/*-------------------------------------------------------------*/
    ...
    private void drawContactLines (ArrayList clist)
    {
        int              i, size;
        DBContactLine    dbcl;
        XYZPolyline      pxyz;
        ...
        size = clist.size ();
        for (i=0; i<size; i++) {
            dbcl = (DBContactLine)clist.get (i);
            if (dbcl == null) {
                continue;
            }
...
/*-------------------------------------------------------------*/
    ...
    private void drawDrapeLines (ArrayList dlist)
    {
        int              i, size;
        DBDrapeLine      dbcl;
        XYZPolyline      pxyz;
...
/*-------------------------------------------------------------*/
    ...
    private void drawExtendedFaults (ArrayList flist)
    {
        int              i, size;
        J3DFault    fp;
        TriMesh          tmesh;
...
/*-------------------------------------------------------------*/
    ...
    private void drawProtoPatches (ArrayList flist)
    {
        int              i, size;
        J3DHorizonPatch  hp;
        TriMesh          tmesh;
