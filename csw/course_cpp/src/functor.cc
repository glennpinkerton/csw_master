...
// A functor is a class or struct where the () operator is 
// overloaded.  The method for the () overload can return 
// any type and can have any number of params.  The class
// or struct can have other stuff in it as well.  In this
// example, there is a tlen variable that can be set to
// "tune" how the matching is done.  The use of accessors
// i.e. SetTlen() can conflict with the overload of () if
// the accessor has the same parameters as the overload.

// An instance of the functor can be passed around and run
// like:   retval = functor(params)
...
class PlayerFtor
{
  ...
  public:
    ...
    void SetTlen (unsigned int tl) {tlen = tl;};

    bool operator() (const std::string txt) {
      if (txt.size() > tlen) return true;
      return false;
    }
};


void FilterPlayerList (std::vector<Player *> &all_players,
                       PlayerFtor &ftor)
{
  for (const auto pp : all_players) {
    std::string nm = pp->GetName();
    if (ftor(nm)) {
      std::cout << "ftor name = " << pp->GetName() << std::endl;
    }
  }
}
...
// Need to put pointers to abstract base into vector
    ...
    PlayerFtor  ftr;
    std::cout << std::endl << "From PlayerFtor len 3" << std::endl;
    FilterPlayerList (all_players, ftr);

    ftr.SetTlen(2);
    std::cout << std::endl << "From PlayerFtor len 2" << std::endl;
    FilterPlayerList (all_players, ftr);
    
    ftr.SetTlen(4);
    std::cout << std::endl << "From PlayerFtor len 4" << std::endl;
    FilterPlayerList (all_players, ftr);
