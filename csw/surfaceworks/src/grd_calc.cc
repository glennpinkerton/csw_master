...
MSL
    ...
    /*
        A lot less workspace is needed for a blended grid.
    */
        else {
...
MSL
            ...
            nmax = 2 * Nccol * Ncrow + Ncol * Nrow;
MSL
            Icoarse = (char *)csw_Malloc (nmax * sizeof(char));
            if (!Icoarse) {
                grd_utils_ptr->grd_set_err (1);
                return -1;
            }
...
*/
    ...
    */
        if (ThicknessFlag == 1  &&  BlendFlag == 0) {
...
/*
  ...
  ****************************************************************
  ...
  speed up searching and blending different components of the
  grid calculations.
...
int CSWGrdCalc::FreeMem (void)
{
    ...
    csw_Free (Grid);
    csw_Free (Icoarse);
    csw_Free (BlendedNeighborIndex);
    csw_Free (BlendedNeighborDistance);
    csw_Free (ExtraPoints);

    DataTable = NULL;
    Grid = NULL;
    Icoarse = NULL;
    BlendedNeighborIndex = NULL;
    BlendedNeighborDistance = NULL;
    ExtraPoints = NULL;
    Nextra = 0;
...
/*
  ...
  ****************************************************************
    ...
    average and the plane fit estimates are blended depending on how
...
/*
  ...
  ****************************************************************
  ...
  data, the trend value is blended with the current value.
...
/*
    Loop through each coarse node, assigning or blending
...
*/
    for (i=0; i<Nrow; i+=Ncoarse) {
        ...
        */
        ...
        /*
            Blend the previously calculated value with the
...
/*
  ...
  ****************************************************************
  ...
  value is blended with the original value depending upon the
  closest data point to the node and the smoothing factor.  A
  radial smoothing estimate is blended with an estimate based
...
*/
    for (i=0; i<Nrow; i+=Ncoarse) {
        ...
        */
        ...
        /*
            Blend the smoothed and original value according to
...
/*
 * Blend the inverse distance error with the error of the closest
...
/*
  ...
  ****************************************************************
  ...
  which the trend elevation is blended with the local elevation.
...
/*
    If the trend almost exactly fits the data, blend all nodes.
*/
    TrendSmoothFlag = 0;
    if (UseShapeGrid == 0) {
    ...
    /*
        Trend nearly fits the data, so set the minimum blending
    ...
    */
    ...
    /*
        Generally calculate the minimum blending distance,
...
int CSWGrdCalc::grd_set_blended_background (int bg)
{
    BlendedBackground = bg;
    BlendedBackground = -1;
    return 1;
}
...
/*
  ****************************************************************
           ...
           g r d _ c a l c _ b l e n d e d _ g r i d

  ****************************************************************

  function name:    grd_calc_blended_grid              (int)

  call sequence:    grd_calc_blended_grid (x, y, z, npts,
                                           grid, mask, ncol, nrow,
                                           x1, y1, x2, y2, options)

  purpose:          Calculate a grid that mixes or blends values.
                    This is like a step grid where the transitions
                    between steps are gradual rather than sharp.

                    The output grid is fundamentally different from
                    a regular grid or a step grid.  The output is an
                    array of CSW_BlendedNode structures.  Each structure
                    has the values and percents of mixing of each value
                    at the node. The blended grid needs to be converted
                    to a regular single valued grid using the function
                    grd_SeparateBlendedGrid prior to display.
  ...
  errors:           1 = memory allocation error
                    ...
                    7 = number of points actually in the data area
                        is less than 1
                    8 = at least one input z value is outside the
  ...
  calling parameters:
    ...
    x        r    CSW_F*    Array of x coordinates
    y        r    CSW_F*    Array of y coordinates
    zin      r    int*          Array of z values at the x,y coordinates
    npts     r    int           Number of points in x, y, z and error.
    grid     w    CSW_Blended   Array to receive the grid values.
    mask     w    char*         Optional array to receive a mask specifying
                                whether a grid node is outside the data
                                area, inside and well defined, or inside
                                and poorly defined.  A well defined node
    ...
    x2       r    CSW_F     Maximum x coordinate of the grid.
    y2       r    CSW_F     Maximum y coordinate of the grid.
    options  r    GRidCalcOptions*    Pointer to structure that has options
                                      for this calculation.  Only the distance_power
                                      and work_margin members are used.

*/

int CSWGrdCalc::grd_calc_blended_grid
      (CSW_F *x, CSW_F *y, int *zin, int npts,
       CSW_Blended *gridout, char *mask, int ncol, int nrow,
       CSW_F x1, CSW_F y1, CSW_F x2, CSW_F y2,
       GRidCalcOptions *options)
{
    int       i, j, k, k2, uc8, istat, offset;
    int       q1val, q1pct, q2val, q2pct, q3val, q3pct, q4val, q4pct;
    int       start, end, iend, nlist, list[MAX_LOCAL + 4], nc, nc2;
    int       *z = NULL;
    CSW_F     xspace, yspace, x0, y0;
    CSW_Blended   *grid = NULL;
    char      *done = NULL;
    int       do_write;
    FAultLineStruct  *faults = NULL;
    int       nfaults;
...
*/
    ...
    if (gridout == NULL  ||  x == NULL  ||  y == NULL  ||  zin == NULL) {
        grd_utils_ptr->grd_set_err (2);
        return -1;
    }
...
/*
 * If there are no points, all nodes are set to zero fraction
 * and the function  returns with error set to 6.
 */
    for (i=0; i<ncol*nrow; i++) {
        gridout[i].v1 = 0;
        gridout[i].p1 = 0;
...
/*
    Allocate space for modified z values.  The input
    z values must be 0 to 254, but the actual grid
    values are 1 to 255.  A temporary work array is
    allocated and filled with the modified values.
*/
MSL
    z = (int *)csw_Malloc (npts * sizeof(int));
    if (!z) {
        grd_utils_ptr->grd_set_err (1);
        return -1;
    }
...
/*
    Only a subset of options are used for blended grids.
*/
    if (options) {
        DistancePower = options->distance_power;
        WorkMargin = options->work_margin;
        ...
        WorkMargin = OptWorkMargin;
        OutsideBoundaryMargin = OptOutsideBoundaryMargin;
        InsideBoundaryMargin = OptOutsideBoundaryMargin;
        FaultedFlag = OptFaultedFlag;
    }
    if (DistancePower < 0) DistancePower = 0;
    if (DistancePower > 6) DistancePower = 6;
    if (WorkMargin < 0) WorkMargin = -1;
    LocalSearchPattern = GRD_OCTANT_SEARCH;
    AnisotropyFlag = 0;
    PreferredStrike = -1000.0f;
...
/*
    Check the points and shift them to be 1 to 255.
*/
    for (i=0; i<npts; i++) {
        z[i] = zin[i] + 1;
        if (z[i] < 1  ||  z[i] > 255) {
...
/*
    Initialize the output mask and grid in case of errors
    or other extraordinary situations.
*/
    uc8 = sizeof(CSW_Blended);
    memset (gridout, 0, uc8*ncol*nrow);
    if (mask) {
        memset (mask, 0, ncol * nrow * sizeof(char));
    }

/*
    Check if all the values are the same.  If so, then all
    grid nodes are set to 100 percent of the only value.

    This is only done if there are no faults, because "faults"
    may be boundaries outside of which no data is available
    and we don't want to set the grid nodes in those areas.
*/
    istat = csw_CheckIntRange (z, npts);
    if (istat == 0  &&  FaultedFlag == 0) {
        for (i=0; i<ncol*nrow; i++) {
            gridout[i].v1 = (unsigned char)(z[0]);
            gridout[i].p1 = 255;
        }
        return 1;
    }
...
/*
    allocate work grid memory
*/
MSL
    grid = (CSW_Blended *)csw_Calloc (Ncol*Nrow*2*sizeof(CSW_Blended));
    if (!grid) {
        grd_utils_ptr->grd_set_err (1);
        return -1;
    }
    done = (char *)(grid + Ncol * Nrow);
...
/*
    Each point input has its nearest non equal valued neighbor
    and the distance to that neighbor saved in work space arrays
    that are allocated here.
*/
MSL
    BlendedNeighborIndex = (int *)csw_Malloc (npts * sizeof(int));
    if (!BlendedNeighborIndex) {
        return -1;
    }

MSL
    BlendedNeighborDistance = (CSW_F *)csw_Malloc (npts * sizeof(CSW_F));
    if (!BlendedNeighborDistance) {
        return -1;
    }

/*
    Calculate the closest non equal valued neighbor and the distance
    to that neighbor at each input data point.
*/
    BuildBlendedNeighbors ();

/*
 * If there a far more grid nodes than points, a different
 * point lookup is used.
 */
    SparseFlag = 0;
    if (Ndata < MAX_BLENDED_NEIGHBORS  &&  ncol * nrow > 100 * Ndata) {
        SparseFlag = 1;
    }
    if (Ndata < MAX_LOCAL / 2) {
        SparseFlag = 1;
    }

/*
    At each coarse node, find the closest data point in each
    quadrant and blend them together to get up to four values
    and percents at each node.
*/
    for (i=0; i<nrow; i+=Ncoarse) {
        offset = i * ncol;
        y0 = y1 + i * yspace;
        for (j=0; j<ncol; j+=Ncoarse) {
            ...
            CollectBlendPoints (i, j, x0, y0, start, end,
                                list, &nlist);
            BlendValues (i, j,
                         list, nlist, x0, y0,
                         &q1val, &q1pct, &q2val, &q2pct,
                         &q3val, &q3pct, &q4val, &q4pct);
            grid[k].v1 = (unsigned char)q1val;
            grid[k].p1 = (unsigned char)q1pct;
            grid[k].v2 = (unsigned char)q2val;
            grid[k].p2 = (unsigned char)q2pct;
            grid[k].v3 = (unsigned char)q3val;
            grid[k].p3 = (unsigned char)q3pct;
            grid[k].v4 = (unsigned char)q4val;
            grid[k].p4 = (unsigned char)q4pct;
        }
    }

    do_write = csw_GetDoWrite ();
    if (do_write) {
        grd_fileio_ptr->grd_write_blended_grid (
            grid, ncol, nrow, Ncoarse,
            x1, y1, x2, y2,
            (char *)"blend1.xyz");
    }

    if (BlendedBackground >= 0) {
        for (i=0; i<nrow; i+=Ncoarse) {
            offset = i * ncol;
            for (j=0; j<ncol; j+=Ncoarse) {
                k = offset + j;
                if (done[k] == 0  ||
                    grid[k].p1 < 1) {
                    grid[k].v1 = (unsigned char)BlendedBackground;
                    grid[k].p1 = (unsigned char)255;
                    grid[k].v2 = (unsigned char)0;
                    grid[k].p2 = (unsigned char)0;
                    grid[k].v3 = (unsigned char)0;
                    grid[k].p3 = (unsigned char)0;
                    grid[k].v4 = (unsigned char)0;
                    grid[k].p4 = (unsigned char)0;
                    done[k] = 1;
                }
            }
        }
...
/*
    Smooth nodes far from data.
*/
    nc = Ncoarse;

    istat = grd_utils_ptr->grd_expand_blended_values (grid, Ncol, Nrow, nc, done,
                                       FaultedFlag, ClosestFault);
    if (istat == -1) {
        return -1;
    }
    ...
    if (do_write) {
        grd_fileio_ptr->grd_write_blended_grid (
            grid, ncol, nrow, Ncoarse,
            x1, y1, x2, y2,
            (char *)"blendexpand.xyz");
    }

    istat = SmoothBlendedGrid (grid, Ncol, Nrow, nc);
    if (istat == -1) {
        return -1;
    }
    istat = SmoothBlendedGrid (grid, Ncol, Nrow, nc);
    ...
    if (do_write) {
        grd_fileio_ptr->grd_write_blended_grid (
            grid, ncol, nrow, Ncoarse,
            x1, y1, x2, y2,
            (char *)"blendsmooth.xyz");
    }

/*
    Refine the coarse grid.  If all 4 corners of a coarse grid are
    each 100 percent of the same value, all of the interior of the
    grid is set to the same.  If the corners differ, the new nodes
    need to be calculated by collecting points as done above.
*/
    nc2 = nc / 2;
    if (nc2 < 1) nc2 = 1;
    ...
    /*
        Fill in if homogeneous throughout the cell.
    */
        for (i=0; i<Nrow-1; i+=nc) {

            offset = i * Ncol;

            for (j=0; j<Ncol-1; j+=nc) {

                k = offset + j;
            /*
             * Bug 9255
             *  If the node is less than or equal to  nc*2+1 cells from a fault, don't
             *  fill in homogeneously.  The fault can affect the homogeneity inside
             *  the cell being filled.  I add one to the critical distance in case
             *  of grazing effects.
             */
                if (FaultedFlag) {
                    if (ClosestFault != NULL  &&  ClosestFault[k] <= 2 * nc + 1) {
                        continue;
                    }
                }
                start = ClosestPoint[k];
                if (start < 0) start = 0;
                if (start >= nc2) {
                    istat = grd_utils_ptr->grd_fill_blended_cell (grid, done, i, j, nc, Ncol);
                }
            }
        }

    /*
        Calculate using local point search if not filled in yet.
    */
        nc /= 2;
        for (i=0; i<Nrow; i+=nc) {
...
*/
    ...
    for (i=RowZero; i<RowZero+OrigNrow; i++) {
        ...
        k2 = j * OrigNcol;
        j++;
        memcpy (gridout+k2, grid+k, OrigNcol*uc8);
        if (mask) {
            memcpy (mask+k2, NodeMask+k, OrigNcol*sizeof(char));
        }
    }
...
}  /*  end of function grd_calc_blended_grid  */
 ...
 *****************************************************************
         ...
         g r d _ r e s a m p l e _ b l e n d e d _ g r i d

 *****************************************************************

  Resample a blended grid to a new geometry.

*/

int CSWGrdCalc::grd_resample_blended_grid
      (CSW_BlendedNode *gridin, int ncol, int nrow,
       CSW_F x1, CSW_F y1, CSW_F x2, CSW_F y2,
       FAultLineStruct *faults, int nfaults,
       ...
       CSW_F x2out, CSW_F y2out)
{
    ...
    CSW_F                x, y, xsp2, ysp2;
    int                  i, j, k, offset, ic;
    int                  i1, i2, j1, j2, offset2, undone,
                         ii, jj, kk, ndo, istat;
    char                 *cdone = NULL;
    int                  firstflag;
 ...
 */
    ...
    if (ncol < 2  ||  nrow < 2  ||  ncout < 2  ||  nrout < 2) {
        grd_utils_ptr->grd_set_err (3);
        return -1;
    }

    if (x2 <= x1  ||  y2 <= y1  ||
        x2out <= x1out  ||  y2out <= y1out) {
        grd_utils_ptr->grd_set_err (4);
    }

    cdone = (char *)csw_Calloc (ncout * nrout * sizeof(char));
    if (cdone == NULL) {
        grd_utils_ptr->grd_set_err (1);
        return -1;
    }
    ...
    for (i=0; i<nrout; i++) {
        ...
        for (j=0; j<ncout; j++) {
            k = offset + j;
            x = x1out + j * xsp2;
            istat = grd_utils_ptr->grd_interpolate_blended_grid (gridin, ncol, nrow,
                                                  x1, y1, x2, y2,
            ...
            if (istat == -1) {
                ...
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                gridout[k].v1 = (unsigned char)ic;
                ic = mix.value2 + 1;
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                gridout[k].v2 = (unsigned char)ic;
                ic = mix.value3 + 1;
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                gridout[k].v3 = (unsigned char)ic;
                ic = mix.value4 + 1;
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                gridout[k].v4 = (unsigned char)ic;

                ic = (int)(mix.percent1 * 2.55f + 1.5f);
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                gridout[k].p1 = (unsigned char)ic;
                ic = (int)(mix.percent2 * 2.55f + 1.5f);
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                if (gridout[k].v2 > 254) ic = 0;
                gridout[k].p2 = (unsigned char)ic;
                ic = (int)(mix.percent3 * 2.55f + 1.5f);
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                if (gridout[k].v3 > 254) ic = 0;
                gridout[k].p3 = (unsigned char)ic;
                ic = (int)(mix.percent4 * 2.55f + 1.5f);
                if (ic < 0) ic = 0;
                if (ic > 255) ic = 255;
                if (gridout[k].v4 > 254) ic = 0;
                gridout[k].p4 = (unsigned char)ic;
                cdone[k] = (char)1;
            }
        }
    }
...
/*
 * fill in any "null" blended nodes from nearest neighbors.
 */
    ndo = 0;
    for (;;) {
        undone = 0;
            ...
            for (j=0; j<ncout; j++) {
                ...
                for (ii=i1; ii<=i2; ii++) {
                    ...
                    for (jj=j1; jj<=j2; jj++) {
                        ...
                        if (kk == k) continue;
                        if (cdone[kk] == 1) {
                            memcpy (gridout+k, gridout+kk, sizeof(CSW_BlendedNode));
                            cdone[k] = 1;
                            break;
                        }
                    }
...
}  /* end of function grd_resample_blended_grid */
...
/*
  ****************************************************************
                       ...
                       B l e n d V a l u e s

  ****************************************************************

    Blend the closest values in each quadrant for a blended grid.

*/

int CSWGrdCalc::BlendValues
       (int irow, int jcol,
        int *list, int nlist, CSW_F x0, CSW_F y0,
        int *v1, int *p1, int *v2, int *p2,
        int *v3, int *p3, int *v4, int *p4)
{
    double       xloc[MAX_LOCAL], yloc[MAX_LOCAL], d1, d2, d3, d4,
                 d11, d22, d33, d44, dt, dist, xt, yt;
    double       dchk, dwgt;
    double       dist_min, dist_local, dist_local2;
    int          zloc[MAX_LOCAL], i, minq1, minq2, minq3, minq4,
                 z1, z2, z3, z4, nq, xq;
    int          pstat, k, istat, ii;
    double       x1, y1, x2, y2, pdist;
    int          val_list[256];
    CSW_F        fault_weight;
    double       ax1, ay1, ax2, ay2;
    double       dt1, dt2, cpz, sang;
    int          i1, i2;
...
/*
    Initialize some local arrays to suppress lint warnings.
*/
    memset ((char *)xloc, 0, MAX_LOCAL * sizeof(CSW_F));
    memset ((char *)yloc, 0, MAX_LOCAL * sizeof(CSW_F));
    memset ((char *)zloc, 0, MAX_LOCAL * sizeof(CSW_F));
    memset (val_list, 0, 256 * sizeof(int));

/*
    initialize quadrant values in case no points are found in
...
*/
    ...
    d1 = (DXmax - DXmin + DYmax - DYmin) * 5.0;
    d2 = d1;
    d3 = d1;
    d4 = d1;

    dist_min = (Xmax - Xmin + Ymax - Ymin) / SAME_POINT_DIVISOR;
    dist_local = (Xspace + Yspace) / 3.0f;
    dist_local2 = (Xspace + Yspace);

/*
    populate the local data set.
*/
    k = 0;
    for (i=0; i<nlist; i++) {
        ii = list[i];
...
*/
    ...
    if (minq1 >= 0) {
        if (d1 <= dist_local  &&  d2 > dist_local2  &&
            d3 > dist_local2  &&  d4 > dist_local2) {
            *v1 = zloc[minq1];
            *p1 = 255;
            return 1;
        }
    ...
    if (minq2 >= 0) {
        if (d2 <= dist_local  &&  d1 > dist_local2  &&
            d3 > dist_local2  &&  d4 > dist_local2) {
            *v1 = zloc[minq2];
            *p1 = 255;
            return 1;
    ...
    if (minq3 >= 0) {
        if (d3 <= dist_local  &&  d2 > dist_local2  &&
            d1 > dist_local2  &&  d4 > dist_local2) {
            *v1 = zloc[minq3];
            *p1 = 255;
            return 1;
        }
    ...
    if (minq4 >= 0) {
        if (d4 <= dist_local  &&  d2 > dist_local2  &&
            d3 > dist_local2  &&  d1 > dist_local2) {
            *v1 = zloc[minq4];
            *p1 = 255;
            return 1;
        }
...
/*
 * If a quadrant was not found, try to find the nearest point in the extra list.
 */
    xq = -1;
    if (minq1 < 0) {
        for (i=0; i<Nextra; i++) {
            xt = Xdata[ExtraPoints[i]] - x0;
            yt = Ydata[ExtraPoints[i]] - y0;
            if (xt > 0.0  &&  yt > 0.0) {
                dist = xt*xt + yt*yt;
                if (dist < d1) {
                    xq = ExtraPoints[i];
...
/*
 * Get the values of the closest points in each quadrant.
 */
    nq = 0;
    if (minq1 >= 0) {
        z1 = zloc[minq1];
...
/*
 * If more than one point has the same value, use the distance
 * between the node and the line connecting the points rather
 * than the point distance if the line distance is smaller.
 *
 * This was put in to make contour style data look better, but
 * in examining bug 8054, it makes scattered style data worse.
 * This is modified to look at the actual distance as well as
 * the perpendicular distance.  If it is contour type data,
 * the actual distance should always be fairly small (I use
 * 3 grid cell sizes as a cuttoff).  If either point is farther
 * than the cutoff, the distance is not changed.
 */
    dchk = (Xspace + Yspace) * 1.5;
    dwgt = 0.0;
    if (z1 != 0  &&  minq1 >= 0) {
        x1 = xloc[minq1];
        y1 = yloc[minq1];
        if (z2 == z1  &&  minq2 >= 0) {
            x2 = xloc[minq2];
            y2 = yloc[minq2];
            pstat = gpf_perpdistance2 (x1, y1, x2, y2,
                                       0.0, 0.0, &pdist);
            if (pstat == 1  &&  d1 < dchk  &&  d2 < dchk) {
                if (pdist < dist_min) {
                    pdist = dist_min;
                }
                if (pdist < d1) {
                    d1 = (pdist + dwgt * d1) / (1.0 + dwgt);
                }
                if (pdist < d2) {
                    d2 = (pdist + dwgt * d2) / (1.0 + dwgt);
                }
            }
        }
        if (z3 == z1  &&  minq3 >= 0) {
...
/*
 * For points in adjacent quadrants that have different values,
 * if the points have a small angle between them, the farther
 * point is weighted even less than its distance dictates.
 * This reduces the problem of having a column or row of lightly
 * weighted values when points are nearly the same angle but
 * in different quadrants.  The angle is calculated by the cross
 * product divided by the product of the vector lengths.  This
 * is actually the sin of the angle, which is good enough for
 * weighting the two points.
 */
    if (minq1 >= 0  &&  minq2 >= 0) {
        i1 = (int)(z1 + .01);
        i2 = (int)(z2 + .01);
...
/*
 * Invert the distances for weighting.
 */
    d1 = 1.0f / d1;
    d2 = 1.0f / d2;
    d3 = 1.0f / d3;
...
/*
    blend the chosen points based on the node's
...
*/
    ...
    *v1 = z1;
    if (z1 > 0) *p1 = (int)(d1/dt*255.0f + 0.5f);
    *v2 = z2;
    if (z2 > 0) *p2 = (int)(d2/dt*255.0f + 0.5f);
    *v3 = z3;
    if (z3 > 0) *p3 = (int)(d3/dt*255.0f + 0.5f);
    *v4 = z4;
    if (z4 > 0) *p4 = (int)(d4/dt*255.0f + 0.5f);

/*
    combine percentages for identical values.
*/
    if (z1 > 0) {
        if (z2 == z1) {
            *p1 += *p2;
...
/*
    If there is only one quadrant, put the value in the first slot.
*/
    if (nq == 1) {
        if (*v2 != 0) {
            *v1 = *v2;
...
/*
    If there is only one value, put it in the first slot
*/
    nq = 0;
    z1 = *v1;
    if (z1 == 0) {
    ...
    if (*v1 != 0  &&  *v1 != z1) return 1;
    if (*v2 != 0  &&  *v2 != z1) return 1;
    if (*v3 != 0  &&  *v3 != z1) return 1;
    if (*v4 != 0  &&  *v4 != z1) return 1;
...
}  /*  end of private BlendValues function  */
...
/*
  ****************************************************************
               ...
               S m o o t h B l e n d e d G r i d

  ****************************************************************

    Smooth grid nodes far from data points by averaging and percentages
  of the same values found in surrounding nodes.

*/

int CSWGrdCalc::SmoothBlendedGrid
         (CSW_Blended *grid, int ncol, int nrow, int nc)
{
    CSW_Blended         *work = NULL;
    int                 i, j, k, ii, jj, i1, i2, j1, j2, kk, k2, cp,
                        uc8, nc2, n0, n, offset, off2, z1, psum, pval;
    CSW_F               fwgt;
    int                 closef, istat1;
...
*/
    uc8 = sizeof(CSW_Blended);
MSL
    work = (CSW_Blended *)csw_Malloc (ncol*nrow*uc8);
    if (!work) {
        return -1;
    }
    csw_memcpy (work, grid, ncol*nrow*uc8);
...
*/
    ...
    /*
        loop through the nodes in a row
    */
        for (j=0; j<ncol; j+=nc) {
            k = offset + j;
            cp = ClosestPoint[k];
     ...
     * bug 8054
     *
     * Do not smooth any grid nodes that have 100 pct of a
     * single value if the node is close to a data point.
     */
            if (cp < nc * 2) {
                if (grid[k].p1 >= 254) {
                    work[k].p1 = grid[k].p1;
            ...
            if (FaultedFlag  &&  ClosestFault != NULL) {
                closef = 1;
                if (ClosestFault[k] > nc) {
                    closef = 0;
                    istat1 = 0;
                }
            }
        ...
        /*
            Average the 4 elements of each node
        */
            for (kk=0; kk<4; kk++) {
            ...
            /*
                average same values from surrounding nodes
            */
                for (ii=i1; ii<=i2; ii+=nc) {
                    off2 = ii*ncol;
                    for (jj=j1; jj<=j2; jj+=nc) {
                        if (closef) {
                            istat1 = grd_fault_ptr->grd_check_grid_fault_blocking (j, i, jj, ii, &fwgt);
                            if (istat1 != 0) {
                                continue;
                            }
                        }
                ...
                if (psum < 0) psum = 0;
                if (n > 0) {
                    pval = (int)(sqrt ((double)psum / (double)n));
                }
                if (pval < 0)
                    pval = 0;
                if (pval > 255)
                ...
                if (kk == 0) {
                    work[k].v1 = (unsigned char)z1;
                    work[k].p1 = (unsigned char)pval;
                }
                else if (kk == 1) {
                    work[k].v2 = (unsigned char)z1;
                    work[k].p2 = (unsigned char)pval;
                }
                else if (kk == 2) {
                    work[k].v3 = (unsigned char)z1;
                    work[k].p3 = (unsigned char)pval;
                }
                else if (kk == 3) {
                    work[k].v4 = (unsigned char)z1;
                    work[k].p4 = (unsigned char)pval;
                }
            }

        /*
            normalize the percentages so they add to 255
        */
            n = work[k].p1 + work[k].p2 + work[k].p3 + work[k].p4;
            if (n > 0) {
                work[k].p1 = (unsigned char)((int)(work[k].p1) * 255 / n);
                work[k].p2 = (unsigned char)((int)(work[k].p2) * 255 / n);
                work[k].p3 = (unsigned char)((int)(work[k].p3) * 255 / n);
                work[k].p4 = (unsigned char)((int)(work[k].p4) * 255 / n);
            }
...
*/
        ...
        }  /*  end of loop through a row  */

    }  /*  end of loop through rows  */

    csw_memcpy (grid, work, ncol*nrow*uc8);

    return 1;

}  /*  end of private SmoothBlendedGrid function  */
...
/*
  ****************************************************************
            ...
            B u i l d B l e n d e d N e i g h b o r s

  ****************************************************************

    Calculate the nearest non equal valued neighbor and the distance
  to that neighbor at each input data point.  This is only done for
  relatively small data sets.  The information is used to minimize
  the contribution of noisy data far from the data.  With large data
  sets, the noisy input data will not be found in the local searchs
  far from it, so the calculation done here is not needed.

    Since this is only done for small data sets, I use a brute force
  n squared approach to the problem.

*/

int CSWGrdCalc::BuildBlendedNeighbors (void)
{
    int          i, j, z0;
    CSW_F        x0, y0, dx, dy, dt, dmin, dist_min;

/*
    Fill all arrays with "unknown" values
*/
    for (i=0; i<Ndata; i++) {
        BlendedNeighborIndex[i] = -1;
        BlendedNeighborDistance[i] = 1.e30f;
    }

/*
    If the data set is large, leave the arrays "unknown"
*/
    if (Ndata > MAX_BLENDED_NEIGHBORS)
        return 1;

    dist_min = (Xmax - Xmin + Ymax - Ymin) / SAME_POINT_DIVISOR;

/*
    loop through points and find the closest neighbor with
    a different value at each point
*/
    for (i=0; i<Ndata; i++) {
        ...
        for (j=0; j<Ndata; j++) {
            ...
            if (dt < dmin) {
                dmin = dt;
                BlendedNeighborIndex[i] = j;
                BlendedNeighborDistance[i] = dmin;
            }
...
}  /*  end of private BuildBlendedNeighbors function  */
...
/*
  ...
  ****************************************************************
  ...
  in blending smooth and original data.
...
/*
    Use local differences blended with the global average to
...
/*
  ...
  ****************************************************************************
    ...
    Blend the difference grid, the shape grid and the current coarse
...
/*
  ****************************************************************
              ...
              C o l l e c t B l e n d P o i n t s
...
int CSWGrdCalc::CollectBlendPoints
              (int i, int j,
               double x0, double y0,
               int start, int end,
               int *listout, int *nlist)

{
    int              jj, ii, i0, i2, j0, j2, level, istat,
                     offset, nt, ipt, ival, maxval, maxloc;
    int              list[MAX_LOCAL];
    int              value_list[256];
    int              minimum_points;

/*
    Initialize output just in case.
*/
    memset (list, 0, MAX_LOCAL * sizeof(int));
    memset (value_list, 0, 256 * sizeof(int));
    *nlist = 0;

/*
 * If the grid is very sparse, it is faster to do
 * a brute force search for the closest point in each quadrant.
 */
    if (SparseFlag == 1) {
        istat = CollectBruteForceBlend (x0, y0, listout, nlist);
        return istat;
    }
...
/*
 * The fix for bug 7901 is here.  Before the fix, the
 * end value was set to MaxNodeDistance (100000 by
 * default) and slowed things down a lot!!
 */
    end = Ncol + Nrow;
    if (end < 20) end = 20;

/*
 * Collect more points if faulted, to make it more probable
 * that at least one is on the correct side of the fault.
 */
    if (FaultedFlag) {
        maxloc *= 4;
        maxval *= 2;
...
/*
    a goto to this label is used in the search code above in case
    the maximum number of points is exceeded.
*/
 ...
 * Bug 8890
 *
 *  If too few points were found, do a brute force search
 *  through the data points to add to the set.  For faulted grids,
 *  this is done twice to increase the number of points found.
 */
    if (nt < minimum_points) {
        FindBlendedPointsFromData (i, j, list, MAX_LOCAL, &nt);
        if (FaultedFlag) {
            FindBlendedPointsFromData (i, j, list, MAX_LOCAL, &nt);
        }
    }
...
}  /*  end of private CollectBlendPoints function  */
...
/*
  ****************************************************************
               ...
               S t r i k e S m o o t h E r r o r s

  ****************************************************************

  Smooth the error grid (Gwork1) and output to Gwork2.
  The smoothing operator is changed for the strike at each node.

*/

int CSWGrdCalc::StrikeSmoothErrors (void)
{

    int       i, j, k, offset, off2, ki, kj, mult, cp;
    int       ndo, ido, cpmax, astat;
    CSW_F     sum, sum2, cwgt, zt;
    CSW_F     x1, y1, x2, y2, st, st0, x, y, stiny, fdum;

    if (FaultedFlag) {
        StrikeSmoothFaultedErrors ();
        return 1;
    }
...
}  /*  end of private StrikeSmoothErrors function  */
...
/*
  ****************************************************************
         ...
         S t r i k e S m o o t h F a u l t e d E r r o r s

  ****************************************************************

  Smooth the faulted error grid (Gwork1) and output to Gwork2.
  The smoothing operator is changed for the strike at each node.

*/

int CSWGrdCalc::StrikeSmoothFaultedErrors (void)
{

    int       i, j, k, offset, off2, ki, kj, cp;
    int       ndo, ido, astat, closef, kflag, istat1;
    CSW_F     sum, sum2, cwgt, zt, fwgt;
    CSW_F     x1, y1, x2, y2, st, st0, x, y, stiny, fdum;
    char      flags[9];
...
}  /*  end of private StrikeSmoothFaultedErrors function  */
#endif
 ...
 ****************************************************************************************
                   ...
                   F i n d B l e n d e d P o i n t s F r o m D a t a

 ****************************************************************************************

  Search the whole x,y data arrays by brute force to find the nearest point
  in each quadrant from the specified x,y location.

*/

void CSWGrdCalc::FindBlendedPointsFromData
              (int irow,
               int jcol,
               int *list,
               int maxlist,
...
{
    int               i, j, n, nin, skip;
    int               closestq1, closestq2, closestq3, closestq4;
    double            d1, d2, d3, d4, dist, dx, dy,
                      xt, yt, xnode, ynode;
    ...
    xnode = jcol * Xspace + Xmin;
    ynode = irow * Yspace + Ymin;
    ...
    if (closestq1 >= 0  &&  n < maxlist) {
        list[n] = closestq1;
        n++;
    }
    if (closestq2 >= 0  &&  n < maxlist) {
        list[n] = closestq2;
        n++;
    }
    if (closestq3 >= 0  &&  n < maxlist) {
        list[n] = closestq3;
        n++;
    }
    if (closestq4 >= 0  &&  n < maxlist) {
        list[n] = closestq4;
        n++;
    }
...
int CSWGrdCalc::CollectBruteForceBlend
        (double x0, double y0, int *listout, int *nlistout)
{
    int        i, n;
    int        minq1, minq2, minq3, minq4;
    double     xt, yt, dist;
    double     dmin1, dmin2, dmin3, dmin4;
    char       *done = NULL;
    ...
    *nlistout = 0;
    ...
    done = (char *)csw_Calloc (Ndata * sizeof(int));
    if (done == NULL) {
        *nlistout = 0;
        return -1;
    }
