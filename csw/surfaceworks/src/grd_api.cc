...
/*
  ...
  ****************************************************************
  ...
  errors:           1 = Error reading from disk.
                    ...
                    7 = The file specified is actually a blended grid file.
                        Use grd_ReadBlendedFile to read it.
 ...
 ******************************************************************************************
               ...
               g r d _ R e s a m p l e B l e n d e d G r i d

 ******************************************************************************************

  function name:  grd_ResampleBlendedGrid                  (int)

  call sequence:  grd_ResampleBlendedGrid (grid, ncol, nrow,
                                           x1, y1, x2, y2,
                                           faults, nfaults,
                                           gridout, ncout, nrout,
                                           x1out, y1out, x2out, y2out)

  purpose:        Resample a subset of an existing blended grid using a different
                  grid geometry.  If the new grid extends outside of the original
                  grid, the nodes outside are made the same value as the nearest
                  node on the original grid.
  ...
  errors:        1 = memory allocation error
                 2 = either grid or newgrid is NULL
                 3 = less than 2 columns or rows specified for either the
                     original or new grid.
                 4 = Either x2 <= x1 or y2 <= y1 or x2out <= x1out or
                     y2out <= y1out.
                 99= The limits of the grid or new grid are too small for
                     the magnitude of the numbers.  Use the double version
                     of this function.
  ...
  calling parameters:
    ...
    grid      r   CSW_BlendedNode*    Input blended grid.
    ncol      r   int                 Number of columns in input grid.
    nrow      r   int                 Number of rows in input grid.
    x1        r   CSW_F           X coordinate of the input grid's
                                      lower left corner.
    y1        r   CSW_F           Y coordinate of the input grid's
                                      lower left corner.
    x2        r   CSW_F           X coordinate of the input grid's
                                      upper right corner.
    y2        r   CSW_F           Y coordinate of the input grid's
                                      upper right corner.
    faults    r   FAultLineStruct*    Array of fault lines used to calculate the
                                      input grid.
    nfaults   r   int                 Number of fault lines.
    gridout   w   CSW_BlendedNode*    Output blended grid.  This must be
                                      allocated by the calling function.
    ncout     r   int                 Number of columns in output grid.
    nrout     r   int                 Number of rows in output grid.
    x1out     r   CSW_F           X coordinate of the output grid's
                                      lower left corner.
    y1out     r   CSW_F           Y coordinate of the output grid's
                                      lower left corner.
    x2out     r   CSW_F           X coordinate of the output grid's
                                      upper right corner.
    y2out     r   CSW_F           Y coordinate of the output grid's
                                      upper right corner.

        Note:  The calling function must allocate an output grid large
               enough to hold ncout * nrout CSW_BlendedNode structures.

*/

int CSWGrdAPI::grd_ResampleBlendedGrid (CSW_BlendedNode *gridin, int ncol, int nrow,
                             CSW_F x1, CSW_F y1, CSW_F x2, CSW_F y2,
                             FAultLineStruct *faults, int nfaults,
                             ...
                             CSW_F x2out, CSW_F y2out)
{
    ...
    istat = csw_CheckRange2 (x1out, y1out, x2out, y2out);
    if (istat == 0) {
        grd_utils_obj.grd_set_err (99);
        return -1;
    }

    istat = grd_calc_obj.grd_resample_blended_grid (gridin, ncol, nrow,
                                       x1, y1, x2, y2,
                                       faults, nfaults,
                                       gridout, ncout, nrout,
                                       x1out, y1out, x2out, y2out);
...
}  /* grd_ResampleBlendedGrid function */
...
/*
  ****************************************************************
             ...
             g r d _ C a l c B l e n d e d G r i d

  ****************************************************************

  function name:    grd_CalcBlendedGrid              (int)

  call sequence:    grd_CalcBlendedGrid (x, y, z, npts,
                                  grid, mask, ncol, nrow,
                                  x1, y1, x2, y2,
                                  faults, nfaults,
                                  options)
  ...
  errors:           1 = memory allocation error
                    ...
                    7 = number of points actually in the data area
                        is less than 1
                    8 = at least one z value is outside of 0-254
                    99 = The range of the x and or y values is too small
                        relative to the values to support CSW_F arithmetic.
  ...
  calling parameters:
    ...
    x        r    CSW_F*    Array of x coordinates
    y        r    CSW_F*    Array of y coordinates
    z        r    int*          Array of z values at the x,y coordinates
    npts     r    int           Number of points in x, y, z and error.
    ...
    x2       r    CSW_F     Maximum x coordinate of the grid.
    y2       r    CSW_F     Maximum y coordinate of the grid.
    faults   r    FAultLineStruct*   Array of fault line structures to use in gridding.
    nfaults  r    int                Number of fault line structures.
    options  r    GRidCalcOptions*   Pointer to structure with options.
                                     Only the distance_power and work_margin
                                     members are used.

*/

int CSWGrdAPI::grd_CalcBlendedGrid (CSW_F *x, CSW_F *y, int *z, int npts,
                         CSW_Blended *grid, char *mask, int ncol, int nrow,
                         CSW_F x1, CSW_F y1, CSW_F x2, CSW_F y2,
                         FAultLineStruct *faults, int nfaults,
                         GRidCalcOptions *options)
{
...
*/
    ...
    istat = grd_calc_obj.grd_calc_blended_grid (x, y, z, npts,
                                   grid, mask, ncol, nrow,
                                   x1, y1, x2, y2, options);
...
}  /*  end of function grd_CalcBlendedGrid  */
...
/*
  ****************************************************************
     ...
     g r d _ C a l c B l e n d e d G r i d F r o m D o u b l e

  ****************************************************************

  function name:    grd_CalcBlendedGridFromDouble              (int)

  call sequence:    grd_CalcBlendedGridFromDouble (x, y, z, npts,
                                  grid, mask, ncol, nrow,
                                  x1, y1, x2, y2,
                                  faults, nfaults, options)
  ...
  purpose:          Calculate a grid that mixes or blends values.
                    ...
                    This is the same as grd_CalcBlendedGrid except it uses
                    double values for geometry.
  ...
  calling parameters:
    ...
    faults   r    FAultLineStruct*   Array of fault line structures to use in gridding.
    nfaults  r    int                Number of fault line structures.
    options  r    GRidCalcOptions*   Pointer to options structure.  Only
                                     the distance_power and work_margin
                                     members are used.


*/

int CSWGrdAPI::grd_CalcBlendedGridFromDouble
                        (double *x, double *y, int *z, int npts,
                         CSW_Blended *grid, char *mask, int ncol, int nrow,
                         double x1, double y1, double x2, double y2,
                         FAultLineStruct *faults, int nfaults,
                         GRidCalcOptions *options)
...
/*
 * If a faulted grid is wanted, npts of zero is ok, so instead
 * of an error, I just return success with all the nodes set to empty.
 */
    for (i=0; i<ncol*nrow; i++) {
        grid[i].v1 = 0;
        grid[i].p1 = 0;
...
/*
    calculate a blended grid using CSW_F x and y coordinates
*/
    istat = grd_calc_obj.grd_calc_blended_grid (xt, yt, z, npts,
                                   grid, mask, ncol, nrow,
...
}  /*  end of function grd_CalcBlendedGridFromDouble  */
...
/*
  ****************************************************************
          ...
          g r d _ S e p a r a t e B l e n d e d G r i d

  ****************************************************************

  function name:    grd_SeparateBlendedGrid           (int)

  call sequence:    grd_SeparateBlendedGrid (grid, ncol, nrow, ival, smfact,
                                             overshoot, range, gridout)

  purpose:          Create a normal grid that has the mixing percentages
                    of the specified ival in the specified blended grid.
                    The returned gridout can be used for anything that
                    a normal grid can be used for (e.g. contouring,
                    smoothing, etc.).  If there is no mixing of the
                    specified ival at a node, the percentage at the node
                    is set to zero.  Other percentages are normalized
                    to values between zero and the specified range.
  ...
  errors:           1 = memory allocation error
                    2 = grid or gridout is NULL
                    3 = ncol or nrow < 2
                    4 = ival must be 0-254
                    5 = no ival values are found in the grid

  calling parameters:

    grid       r    CSW_Blended*  Blended grid data
    ncol       r    int           Number of columns in grid
    nrow       r    int           Number of rows in grid
    ival       r    int           Value to separate out.
    smfact     r    int           Smoothing factor to apply to output grid
                                  (1-9) or zero for no smoothing.
    overshoot  r    int           0 = no overshoot outside 0-range,
                                  1 = overshoot outside 0-range
    range      r    CSW_F         output grid will be scaled between
                                  0 and range.  If range is less than or
                                  equal to zero, the output grid will range
                                  from 0 to 1.0, not including overshoots.
    gridout    w    CSW_F*        output normal grid with mixing percentages

*/

int CSWGrdAPI::grd_SeparateBlendedGrid (CSW_Blended *grid, int ncol, int nrow, int ival,
                             int smfact, int overshoot, CSW_F range, CSW_F *gridout)
{
    int              i, j, k, offset;
    int              itot, n;
    CSW_F            *work = NULL, zeroval;
    CSW_F            *work2 = NULL, midrange, scalefact, orig_range;
    CSW_F            zbase, ztop, low, high;
    int              nbad, nbad_top, nbad_base, ido, ndo, maxbad;
...
/*
    The overshoot parameter is now ignored.
    An iterative overshoot algorithm is implemented
    instead as part of bug 8054 fix.
*/
    overshoot = overshoot;
...
/*
    the grid values range from 1-255, but the original input was
    allowed to be 0-254.  The grid values are each 1 higher than
    the input, so 1 is added to ival here to facilitate the match.
*/
    ival++;

    if (ival < 1  ||  ival > 255) {
        grd_utils_obj.grd_set_err (4);
        return -1;
    }
    ...
    work2 = (CSW_F *)csw_Malloc (ncol * nrow * sizeof(CSW_F));
    if (work2 == NULL) {
        grd_utils_obj.grd_set_err (1);
        return -1;
    }
...
/*
    Find any ival values at any node and calculate
    the percentage at the nodes.
*/
    n = 0;
    for (i=0; i<nrow; i++) {
        ...
        for (j=0; j<ncol; j++) {
            ...
            k = offset + j;
            itot = 0;
            if ((int)(grid[k].v1) == ival) {
                itot += grid[k].p1;
            }
            else if ((int)(grid[k].v2) == ival) {
                itot += grid[k].p2;
            }
            else if ((int)(grid[k].v3) == ival) {
                itot += grid[k].p3;
            }
            else if ((int)(grid[k].v4) == ival) {
                itot += grid[k].p4;
            }
 ...
 * bug 8054
 *
 * Iteratively expand the z values and then smooth.  If
 * only a small number of smoothed values are inside the
 * range when they should be at the limits of the range,
 * then the loop is exited.  For example, a value of 1.0,
 * at the top of the range, is set to 1.05, and then smoothed.
 * If the results of the smoothing at that node are less
 * than 1.0, the smoothed node is "bad".
 */
    ido = 1;
    ndo = 10;
    if (smfact < 1) smfact = 1;
    if (smfact > 9) smfact = 9;
    maxbad = ncol * nrow / 1000;
    if (maxbad < 0) maxbad = 0;
    ztop = 1.05f * range;
    zbase = -.05f * range;
    nbad = 0;
    nbad_top = ncol*nrow;
    ...
    for (;;) {
        scalefact = 1.0f + .05f * (CSW_F)ido;
        for (i=0; i<ncol*nrow; i++) {
            gridout[i] = (work2[i] - midrange) * scalefact + midrange;
            if (nbad_base == 0) {
                if (gridout[i] < zbase) gridout[i] = zbase;
            }
            if (nbad_top == 0) {
                if (gridout[i] > ztop) gridout[i] = ztop;
            }
        }

    /*
     * the plateau grid smoothing is introduced as part of
     * the bug 8054 fix.  If a grid node is outside the limits
     * defined by low and high, its original value is transferred
     * to the smoothed grid rather than its smoothed value.
     */
        grd_calc_obj.grd_smooth_plateau_grid
                       (gridout, ncol, nrow, 1,
        ...
        nbad_base = 0;
        for (i=0; i<ncol*nrow; i++) {
            if (work2[i] >= orig_range  &&  work[i] < orig_range) {
                nbad++;
                nbad_top++;
            }
            if (work2[i] <= 0.0f  &&  work[i] > 0.0f) {
                nbad++;
                nbad_base++;
            }
        }
        if (nbad <= maxbad  ||  ido > ndo) break;
        ido++;
    }

    memcpy (gridout, work, ncol * nrow * sizeof(CSW_F));

    return 1;

}  /*  end of function grd_SeparateBlendedGrid  */
...
/*
  ****************************************************************
              ...
              g r d _ W r i t e B l e n d e d F i l e

  ****************************************************************

  function name:    grd_WriteBlendedFile                (int)

  call sequence:    grd_WriteBlendedFile (filename, comment,
                                          grid, mask,
                                          ncol, nrow,
                                          xmin, ymin, xmax, ymax,
                                          faults, nfaults)

  purpose:          Write the specified blended grid and mask to a portable
                    binary data file.  The gridtype in the file is always set
                    to GRD_BLENDED_GRID_FILE.
  ...
  calling parameters:
    ...
    filename    r    char*           File to write to, relative to the current
                                     directory.
    comment     r    char*           Optional comment up to 200 bytes long.
    grid        r    CSW_Blended*    Grid values array, without hard nulls.
    mask        r    char*           Optional mask array or NULL if not used.
    ncol        r    int             Number of columns in the grid.
    nrow        r    int             Number of rows in the grid.
    xmin        r    double          Minimum x of the grid.
    ...
    xmax        r    double          Maximum x of the grid.
    ymax        r    double          Maximum y of the grid.
    faults      r    FAultLineStruct*  Array of faults used to calculate the grid.
    nfaults     r    int             Number of faults

*/

int CSWGrdAPI::grd_WriteBlendedFile (const char *filename, const char *comment,
                          ...
                          FAultLineStruct *faults, int nfaults)
{
    ...
    istat = grd_fileio_obj.grd_write_blended_file (filename, comment,
                                    grid, mask,
                                    ncol, nrow,
                                    xmin, ymin, xmax, ymax,
                                    faults, nfaults);
...
}  /*  end of grd_WriteBlendedFile function  */
...
/*
  ****************************************************************
              ...
              g r d _ R e a d B l e n d e d F i l e
  ...
  ****************************************************************
  ...
  errors:           1 = Error reading from disk.
                    ...
                    3 = The grid is a null pointer.
                    4 = One of the geometry parameters is a NULL pointer.
                    5 = Error allocating grid or mask data.
                    6 = Unknown grid file version.
                    7 = The grid type is not GRD_BLENDED_GRID_FILE.  Use
                        grd_ReadFile to read this file.
                    8 = This is a multiple grid file.  Use grd_ReadMultipleGrid
                        to read it.
  ...
  calling parameters:
    ...
    filename    r    char*      File to read from, relative to the current
                                directory.
    comment     w    char*      Optional comment at least 200 bytes long.
    grid        w    CSW_Blended** Grid values array, without hard nulls.
    mask        w    char**     Optional mask array or NULL if not used.
    ncol        w    int*       Number of columns in the grid.
    nrow        w    int*       Number of rows in the grid.
    xmin        w    double*    Minimum x of the grid.
    ...
    xmax        w    double*    Maximum x of the grid.
    ymax        w    double*    Maximum y of the grid.
    faults      w    FAultLineStruct**  Array of faults used to calculate the grid.
    nfaults     w    int*       Number of faults.

*/

int CSWGrdAPI::grd_ReadBlendedFile (const char *filename, char *comment,
                         CSW_Blended **grid, char **mask,
                         int *ncol, int *nrow,
                         double *xmin, double *ymin, double *xmax, double *ymax,
                         FAultLineStruct **faults, int *nfaults)
{
    ...
    istat = grd_fileio_obj.grd_read_blended_file (filename, comment,
                                   grid, mask,
                                   ncol, nrow,
                                   xmin, ymin, xmax, ymax,
                                   faults, nfaults);
...
}  /*  end of grd_ReadBlendedFile function  */
...
/*
  ****************************************************************
      ...
      g r d _ I n t e r p o l a t e B l e n d e d G r i d

  ****************************************************************

  function name:  grd_InterpolateBlendedGrid          (int)

  call sequence:  grd_InterpolateBlendedGrid (grid, ncol, nrow,
                                              x1, y1, x2, y2,
                                              faults, nfaults, firstflag,
                                              x, y, range, mixture)

  purpose:        Return the values and the mixing percent of each
                  value at the point specified.
  ...
  errors:         2 = A NULL pointer is specified for grid or mixture.
                  3 = Either ncol or nrow is less than 2.
                  4 = The grid limits are inconsistent (x1 > x2 or y1 > y2).
                  5 = The x,y point is outside of the grid.
                  6 = No values can be found in the nodes surrounding the
                      specified x,y point.
                  99= The grid limits are too close together for the
                      magnitude of the numbers.  Use the double version
                      of this function.

  calling parameters:

    grid         r  CSW_BlendedNode*    Blended grid array.
    ncol         r  int                 Number of columns in grid.
    nrow         r  int                 Number of rows in grid.
    x1           r  CSW_F           minimum x of grid
    y1           r  CSW_F           minimum y of grid
    x2           r  CSW_F           maximum x of grid
    y2           r  CSW_F           maximum y of grid
    faults       r  FAultLineStruct     Array of fault line structures that were
                                        used to calculate the grid.
    nfaults      r  int                 Number of fault line structures.
    firstflag    r  int                 If set to 1, the fault indices are set up
                                        If set to zero, the previous setup is used
                                        If set to -1, the fault index setup is csw_Freed and
                                        nothing else is done.
    x            r  CSW_F           x coordinate to interpolate at
    y            r  CSW_F           y coordinate to interpolate at
    range        r  CSW_F           range for percent data in the
                                        returned mixture.  The sum of all
                                        the mixture "percents" is range.
                                        If less than or equal to zero,
                                        the sum will be 1.0.
    mixture      w  CSW_Mixture*        returned mixture structure

*/

int CSWGrdAPI::grd_InterpolateBlendedGrid (CSW_BlendedNode *grid, int ncol, int nrow,
                                CSW_F x1, CSW_F y1, CSW_F x2, CSW_F y2,
                                FAultLineStruct *faults, int nfaults, int firstflag,
                                CSW_F x, CSW_F y, CSW_F range,
                                CSW_Mixture *mixture)
{
    int                   istat;
    ...
    istat = grd_utils_obj.grd_interpolate_blended_grid (grid, ncol, nrow,
                                          x1, y1, x2, y2,
                                          faults, nfaults, firstflag,
                                          x, y, range, mixture);
...
}  /*  end of function grd_InterpolateBlendedGrid  */
 ...
 ******************************************************************************************
            ...
            g r d _ R e s a m p l e B l e n d e d G r i d F r o m D o u b l e

 ******************************************************************************************

  function name:  grd_ResampleBlendedGridFromDouble        (int)

  call sequence:  grd_ResampleBlendedGridFromDouble (grid, mask, ncol, nrow,
                                           x1, y1, x2, y2,
                                           faults, nfaults,
                                           faults, nfaults,
                                           newgrid, newmask, newncol, newnrow,
  ...
  calling parameters:
    ...
    ncol      r   int                 Number of columns in input grid.
    nrow      r   int                 Number of rows in input grid.
    x1        r   double              X coordinate of the input grid's
                                      lower left corner.
    y1        r   double              Y coordinate of the input grid's
                                      lower left corner.
    x2        r   double              X coordinate of the input grid's
                                      upper right corner.
    y2        r   double              Y coordinate of the input grid's
                                      upper right corner.
    faults    r   FAultLineStruct*    Faults used to calculate the input grid.
    nfaults   r   int                 Number of faults;
    gridout   w   CSW_BlendedNode*    Output blended grid.  This must be
                                      allocated by the calling function.
    ncout     r   int                 Number of columns in output grid.
    nrout     r   int                 Number of rows in output grid.
    x1out     r   double              X coordinate of the output grid's
                                      lower left corner.
    y1out     r   double              Y coordinate of the output grid's
                                      lower left corner.
    x2out     r   double              X coordinate of the output grid's
                                      upper right corner.
    y2out     r   double              Y coordinate of the output grid's
                                      upper right corner.
...
int CSWGrdAPI::grd_ResampleBlendedGridFromDouble (CSW_BlendedNode *gridin, int ncol, int nrow,
                             double dx1, double dy1, double dx2, double dy2,
                             FAultLineStruct *faults, int nfaults,
                             CSW_BlendedNode *gridout, int ncout, int nrout,
                             double dx1out, double dy1out,
                             double dx2out, double dy2out)
{
    int                      istat;
    CSW_F                    x1, y1, x2, y2, x1out, y1out, x2out, y2out;
...
}  /* End of grd_ResampleBlendedGridFromDouble function */
...
/*
  ****************************************************************
  ...
  g r d _ I n t e r p o l a t e B l e n d e d G r i d F r o m D o u b l e

  ****************************************************************

  function name:  grd_InterpolateBlendedGridFromDouble          (int)

  call sequence:  grd_InterpolateBlendedGridFromDouble (grid, ncol, nrow,
                                              dx1, dy1, dx2, dy2,
                                              faults, nfaults, firstflag,
                                              x, y, range, mixture)
  ...
  calling parameters:
    ...
    dx2          r  CSW_F           maximum x of grid
    dy2          r  CSW_F           maximum y of grid
    faults       r  FAultLineStruct*    Array of fault lines sed to calculate the grid.
    nfaults      r  int                 Number of fault lines.
    firstflag    r  int                 If set to 1, the fault indices are set up
                                        If set to zero, the previous setup is used
                                        If set to -1, the fault index setup is csw_Freed and
                                        nothing else is done.
    dx           r  CSW_F           x coordinate to interpolate at
    dy           r  CSW_F           y coordinate to interpolate at
    range        r  CSW_F           range for percent data in the
                                        returned mixture.  The sum of all
                                        the mixture "percents" is range.
                                        If less than or equal to zero,
...
int CSWGrdAPI::grd_InterpolateBlendedGridFromDouble (CSW_BlendedNode *grid, int ncol, int nrow,
                                double dx1, double dy1, double dx2, double dy2,
                                FAultLineStruct *faults, int nfaults, int firstflag,
                                double dx, double dy, CSW_F range,
                                CSW_Mixture *mixture)
{
    int                   istat;
...
}  /*  end of function grd_InterpolateBlendedGridFromDouble  */
...
/*
  ...
  ****************************************************************
  ...
  errors:         1 = Error reading from disk.
                  ...
                  9 = The file is a blended grid file.
...
/*
  ****************************************************************
           ...
           g r d _ G e t B l e n d e d G r i d N o d e

  ****************************************************************

  function name:  grd_GetBlendedGridNode      (int)

  call sequence:  grd_GetBlendedGridNode (grid, ncol, nrow,
                                          col, row,
                                          range, mixture)

  purpose:        Return the values and the mixing percent of each
                  value at the node specified.
  ...
  errors:         2 = A wild value for ncol, nrow, col or row has been
                      specified.
                  3 = A NULL pointer is specified for grid or mixture.
                  4 = Either ncol or nrow is less than 2.
                  5 = Either col or row is outside of the grid limits.
  ...
  calling parameters:
    ...
    ncol         r  int                 Number of columns in grid.
    nrow         r  int                 Number of rows in grid.
    col          r  int                 Column of node to return.
    row          r  int                 Row of node to return.
    range        r  CSW_F           range for percent data in the
                                        returned mixture.  The sum of all
                                        the mixture "percents" is range.
                                        If less than or equal to zero,
...
int CSWGrdAPI::grd_GetBlendedGridNode (CSW_BlendedNode *grid, int ncol, int nrow,
                            int col, int row,
                            CSW_F range, CSW_Mixture *mixture)
{
    int                   istat;

    istat = grd_utils_obj.grd_get_blended_grid_node (grid, ncol, nrow,
                                       col, row,
                                       range, mixture);

    return istat;

}  /*  end of function grd_GetBlendedGridNode  */
...
/*
  ****************************************************************
          ...
          g r d _ M i x t u r e T o B l e n d e d N o d e

  ****************************************************************

  function name:    grd_MixtureToBlendedNode       (int)

  call sequence:    grd_MixtureToBlendedNode (mix, blend)

  purpose:          Convert a mixture structure into a blended node
                    structure.

  return value:     -1 = error
                     1 = success

  errors:           1 = A null pointer was specified.

  calling parameters:

    CSW_Mixture      r   *mix      Pointer to a CSW_Mixture structure.
    CSW_BlendedNode  r   *blend    Pointer to CSW_BlendedNode structure.

*/

int CSWGrdAPI::grd_MixtureToBlendedNode (CSW_Mixture *mix, CSW_BlendedNode *blend)
{
    CSW_F          sum;
    int                nv, p1, p2, p3, p4, isum;

/*
    Note that each value in the mixture must be incremented by
    one to go into the blended node, since zero is a reserved
    value in the internal blended node structure.
*/
    blend->v1 = (unsigned char)(mix->value1 + 1);
    blend->v2 = (unsigned char)(mix->value2 + 1);
    blend->v3 = (unsigned char)(mix->value3 + 1);
    blend->v4 = (unsigned char)(mix->value4 + 1);

    blend->p1 = (unsigned char)0;
    blend->p2 = (unsigned char)0;
    blend->p3 = (unsigned char)0;
    blend->p4 = (unsigned char)0;
    ...
    if (nv > 4) {
        sum += mix->missing_percent;
    }

    p1 = p2 = p3 = p4 = 0;

    p1 = (int)(mix->percent1 / sum * 255.0f + 0.5f);
    if (nv > 1) {
        p2 = (int)(mix->percent2 / sum * 255.0f + 0.5f);
    }
    if (nv > 2) {
        p3 = (int)(mix->percent3 / sum * 255.0f + 0.5f);
    }
    if (nv > 3) {
        p4 = (int)(mix->percent4 / sum * 255.0f + 0.5f);
    }
    ...
    blend->p1 = (unsigned char)p1;
    blend->p2 = (unsigned char)p2;
    blend->p3 = (unsigned char)p3;
    blend->p4 = (unsigned char)p4;

    return 1;

}  /*  end of function grd_MixtureToBlendedNode  */
...
/*
  ****************************************************************
          ...
          g r d _ B l e n d e d N o d e T o M i x t u r e

  ****************************************************************

  function name:    grd_BlendedNodeToMixture       (int)

  call sequence:    grd_BlendedNodeToMixture (blend, mix)

  purpose:          Convert a blended node structure into a mixture
                    structure.
...
int CSWGrdAPI::grd_BlendedNodeToMixture (CSW_BlendedNode *blend, CSW_F range,
                              CSW_Mixture *mix)
{
    int                istat;
    CSW_BlendedNode    grid[4];

    memcpy ((char *)grid, (char *)blend, sizeof(CSW_BlendedNode));

    istat = grd_utils_obj.grd_get_blended_grid_node (grid, 2, 2, 0, 0, range, mix);

    return istat;

}  /*  end of function grd_BlendedNodeToMixture  */
 ...
 ************************************************************************
*/
int CSWGrdAPI::grd_TriMeshBlendedValuesFromGrid (
    NOdeStruct     *nodes,
    int            num_nodes,
    EDgeStruct     *edges,
    int            num_edges,
    ...
    )
{
    ...
    istat =
      grd_triangle_obj.grd_trimesh_blended_values_from_grid (
        nodes,
        num_nodes,
        edges,
        num_edges,
...
int CSWGrdAPI::grd_WriteBlendedGrid
...
{
    int  istat =
           grd_fileio_obj.grd_write_blended_grid
...
int CSWGrdAPI::grd_WriteBlendedTextTriMesh
                 (int vused,
                  double *vbasein,
                  TRiangleStruct *triangles, int ntriangles,
                  EDgeStruct *edges, int nedges,
