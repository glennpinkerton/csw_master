...
static int           ClustPts[MAX_CLUST],
                     ProfPts[MAX_PROF];
static CSW_F     Xsource[MAX_SOURCE],
                     Ysource[MAX_SOURCE],
...
static int           HemisphereFlag = 0,
                     ...
                     BustSign = 0,
                     NumBusts = 0;
static CSW_F     LogBase;

static CSW_F     X[MAX_POINTS], Y[MAX_POINTS], Z[MAX_POINTS];
static int           Ndata;

static CSW_F     Xmin, Ymin, Xmax, Ymax;
static FourFloat     ProfLoc[MAX_PROF], ClustLoc[MAX_CLUST];

static char          Ans[200];

static CSW_F     AnisoAngle = -1000.f,
                     AnisoExcen = 1.0f,
                     Zfudge = 1.0f;
static int           Nxyrow, Nxycol, GridXY;

CSW_F            csw_FloatRandom ();
...

{
    ...
    CSW_F   *x, *y, *z;
    CSW_F   xt, yt, sum, dt, xt2, yt2, zt2, xt3, yt3;
    CSW_F   zmin, zmax, ang, 
    ...
    if (AnisoAngle >= -180.0  &&  AnisoAngle <= 180.0) {
        ...
        sang = (CSW_F)sin ((double)ang);
        cang = (CSW_F)cos ((double)ang);
...

{
    ...
    CSW_F      xmin, ymin, xmax, ymax;
    int            i;
    CSW_F      x0, y0, xt, yt, dt, r1, r2, r0, dr;
    CSW_F      xw, yw;
    double         ang;
    ...
    if (HemisphereFlag == 1) {
        for (i=0; i<npts; i++) {
            ...
            r1 = (CSW_F)sqrt ((double)dt);
            r1 *= r1;
            dt = r2 - r1;
            if (dt < 0.0) dt = 0.0;
            z[i] = (CSW_F)sqrt ((double)dt);
        }
    }
...
{
    int       i, n;
    CSW_F zt, zmin, zmax, zn, zb, zf, pct,
